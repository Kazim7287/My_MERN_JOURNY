...
function ResumeActions({ resume, onActionComplete, onStatusUpdate }) {
  ...
  const handleView = () => navigate(`/?resumeId=${resume._id}`);
      ...
      <div className="action-buttons">
        ...
        {currentStatus === "failed" && (
          <button onClick={() => alert(resume.error || "Processing failed")} className="btn-action btn-error" title="View Error">
            <span className="action-tooltip">Error</span>
            ⚠️
          </button>
        )}
        {isProcessing && (
          ...
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-delete-icon">
              <div className="delete-icon-circle">
                <span className="delete-icon-inner">🗑️</span>
              </div>
            </div>
            ...
            <p className="modal-description">
              This will permanently delete this enhanced resume and all its data.
            </p>
        ...
        <div className="modal-overlay" onClick={() => !loading && setShowReEnhanceModal(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-re-enhance-icon">
              <div className="re-enhance-icon-circle">
                <span className="re-enhance-icon-inner">🔄</span>
              </div>
            </div>
            ...
            <div className="quick-suggestions">
              ...
              <div className="suggestions-list">
                {quickUrls.map((item, i) => (
                  <button
                    key={i}
                    onClick={() => setCompanyUrl(item.url)}
                    className={`suggestion-chip ${companyUrl === item.url ? 'active' : ''}`}
                  >
                    {item.name}
                  </button>
                ))}
            ...
            <div className="modal-buttons">
              <button 
                onClick={() => setShowReEnhanceModal(false)} 
                className="btn-modal-secondary"
                disabled={loading}
              >
                Cancel
